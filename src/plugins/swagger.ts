...
export default fp(async (fastify) => {
  ...
  fastify.addSchema({
    ...
    properties: {
      username: { type: 'string', description: 'Operator username', example: 'operator1' },
      password: { type: 'string', description: 'Operator password', example: 'password123' }
    },
  ...
  fastify.addSchema({
    ...
    properties: {
      amount: { type: 'number', description: 'Purchase amount', example: 100.5 },
  ...
  fastify.addSchema({
    ...
    properties: {
      username: { type: 'string', description: 'Operator username', example: 'operator1' },
      displayName: { type: 'string', description: 'Operator display name', example: 'Jane Smith' }
    },
  ...
  fastify.addSchema({
    $id: 'transaction',
    type: 'object',
    required: ['id', 'type', 'amount', 'loyaltyPercentage', 'userId', 'operatorId'],
    properties: {
      id: { type: 'string', description: 'Transaction ID', example: 'txn_789' },
      type: { type: 'string', enum: ['check-in', 'check-out'], description: 'Transaction type', example: 'check-in' },
      amount: { type: 'number', description: 'Transaction amount', example: 100 },
      checkOutAmount: { type: 'number', description: 'Check-out amount', example: 80 },
