...
import { RecentActivitySchema } from "./recent-activity/recent-activity.schema";
...
export const getDatabaseUrlByMerchantId = (merchantId?: ID): string => {
  if (!merchantId)
    throw new Error("Merchant ID is required for database connection");
...
export const createMongooseClient = (merchantId: ID = "") => {
  if (!merchantId)
    throw new Error("Merchant ID is required for mongoose client");
  if (!clients[merchantId]) {
    ...
    clients[merchantId].model("RecentActivity", RecentActivitySchema),
    ...
    clients[merchantId].on("error", (err) => {
      console.error(
        `MongoDB client connection error for merchant ${merchantId}:`,
        err
      );
    });
  }
