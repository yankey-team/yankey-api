...
const officeOperators: FastifyPluginAsync = async (fastify): Promise<void> => {
  ...
  fastify.get('/operators', {
    ...
    handler: async (request) => {
      ...
      const { data: operators, error } = await operatorModel.findAllByMerchant(request.user.merchantId);
  ...
  // Update operator
  fastify.put<{ Params: { id: string }, Body: OperatorUpdatePayload }>('/operators/:id', {
    schema: {
    ...
    handler: async (request, reply) => {
      ...
      const { data: updated, error } = await operatorModel.updateOperator(id, updateData);
      if (error || !updated) {
  ...
  fastify.delete<{ Params: { id: string } }>('/operators/:id', {
    ...
    handler: async (request, reply) => {
      ...
      const operatorModel = new OperatorModel(request.user.merchantId);
      const { data: deleted, error } = await operatorModel.deleteOperator(id, request.user.merchantId);
      if (error || !deleted) {
        reply.code(404).send({ error: error || 'Operator not found' });
        return;
      }
