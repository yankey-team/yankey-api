...
const officeAuth: FastifyPluginAsync = async (fastify): Promise<void> => {
  fastify.post<{ Body: OfficeSignUpPayload }>("/auth/signup", {
    ...
    handler: async (request, reply) => {
      ...
      const { data: existingMerchant } =
        await merchantModel.getMerchantByDomain(domain);
      if (existingMerchant) {
      ...
      if (merchantResult.error || !merchantResult.data) {
        reply
          .code(500)
          .send({ error: merchantResult.error || "Could not create merchant" });
        return;
      }
      ...
      if (operatorResult.error || !operatorResult.data) {
        reply
          .code(500)
          .send({
            error: operatorResult.error || "Could not create admin operator",
          });
        return;
      }
  ...
  fastify.post<{ Body: OfficeSignInPayload }>("/auth/signin", {
    ...
    handler: async (request, reply) => {
      ...
      const { data: merchant } = await merchantModel.getMerchantByDomain(
        domain
      );
      if (!merchant) {
